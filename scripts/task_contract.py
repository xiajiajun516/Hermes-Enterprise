...
HEADERS = ["Run Identity", "Goal & Scope", "Source of Truth", "Environment SOP", "Artifact I/O Contract", "Checksum / Verification", "Hard Prohibitions", "Final Report Protocol"]
SECTION_LABELS = {
...
def _frontmatter(text):
    ...
    for line in lines:
        if "\t" in line or any(marker in line for marker in "&*!"): raise ValueError("ambiguous YAML")
        if re.match(r"^[^ :]+:", line):
            ...
            if not isinstance(data.get(current), list): raise ValueError("invalid list")
            key, value = line[4:].split(":", 1)
            item = {key: _scalar(value)}
            data[current].append(item)
        elif line.startswith("    ") and item is not None:
            key, value = line.strip().split(":", 1)
            if key in item: raise ValueError("duplicate key")
            item[key] = _scalar(value)
        else: raise ValueError("unsupported YAML")
    return data, text[end + 5:]


def _validate_sections(body):
    matches = list(re.finditer(r"^## (.+)$", body, re.M))
    if [match.group(1) for match in matches] != HEADERS: raise ValueError("exactly eight ordered headings required")
...
def _artifact_identity(path):
    parts = path.split("/")
    if len(parts) != 3 or parts[0] != "artifacts" or parts[1] not in ALLOWED_SLUGS: return None
...
def parse_and_validate_contract(path):
    ...
    required = {"contract_version", "run_id", "created_at_utc", "tier", "stage", "attempt", "agent_display_name", "agent_slug", "parent_run_id", "language", "inputs", "outputs"}
    ...
    if data["created_at_utc"] != iso_utc_for_run_id(data["run_id"]) or not isinstance(data["attempt"], int) or data["attempt"] < 1:
        raise ValueError("invalid identity")
    if not all(isinstance(data[key], str) and data[key] for key in ("tier", "stage", "agent_display_name", "language")) or data["agent_slug"] not in ALLOWED_SLUGS:
        raise ValueError("invalid identity")
    if data["parent_run_id"] is not None and not RUN_RE.fullmatch(str(data["parent_run_id"])): raise ValueError("invalid identity")
    if not isinstance(data["inputs"], list) or not isinstance(data["outputs"], list): raise ValueError("invalid contract lists")
    for item in data["inputs"]:
        identity = _artifact_identity(item.get("path", "")) if isinstance(item, dict) else None
        if set(item) != {"path", "artifact_name", "sha256", "producer_run_id"} or not validate_relative_path(item["path"]) or not SHA_RE.fullmatch(item["sha256"]):
            raise ValueError("invalid input")
        if not identity or item["artifact_name"] != identity[2] or item["producer_run_id"] != identity[1] or not RUN_RE.fullmatch(item["producer_run_id"]):
            raise ValueError("inconsistent input")
    for item in data["outputs"]:
        if set(item) != {"agent_slug", "artifact_name", "target_path", "template", "write_mode"} or item["write_mode"] != "create-new" or not validate_output(item["agent_slug"], item["artifact_name"], item["target_path"]):
...
def _tracked(root, relative_path):
    result = subprocess.run(["git", "-C", str(root), "ls-files", "--error-unmatch", "--", relative_path], text=True, capture_output=True)
